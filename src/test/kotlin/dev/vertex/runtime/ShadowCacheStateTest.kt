...
class ShadowCacheStateTest {
    ...
    fun `reuses stable map and invalidates on meaningful changes`() {
        ...
        assertTrue(cache.needsRender(0f, 1.0, 2.0, 3.0))
        assertFalse(cache.needsRender(0.04f, 1.0, 2.0, 3.0))
        assertTrue(cache.needsRender(0.06f, 1.0, 2.0, 3.0))
        assertTrue(cache.needsRender(0.06f, 1.0, 2.01, 3.0))
        assertFalse(cache.needsRender(0.06f, 1.0, 2.01, 3.0))
        cache.invalidate()
        assertTrue(cache.needsRender(0.06f, 1.0, 2.01, 3.0))
    }
    ...
    fun `section invalidation maps to tiles until a successful draw`() {
        ...
        cache.markRendered(0f, 0.0, 0.0, 0.0)
        ...
        assertTrue(cache.needsRender(0f, 0.0, 0.0, 0.0))
        assertEquals(2, cache.pendingSectionCount())
        cache.markRendered(0f, 0.0, 0.0, 0.0)
        assertTrue(cache.dirtyTiles().isEmpty())
        assertFalse(cache.needsRender(0f, 0.0, 0.0, 0.0))
    }
    ...
    fun `keeps invalidation that arrives during a draw`() {
        ...
        cache.markRendered(0f, 0.0, 0.0, 0.0, renderEpoch)
